...
import com.nukateam.minego.client.models.IStickeredModel;
...
public class StickeredLayer<T extends GeoAnimatable> extends GeoRenderLayer<T> {
    public static HashMap<ResourceLocation, Boolean> textures = new HashMap();

    public StickeredLayer(GeoRenderer<T> entityRenderer) {
        super(entityRenderer);
    }

    protected boolean resourceExists(ResourceLocation location) {
        return (Boolean)textures.computeIfAbsent(location, ResourceUtils::resourceExists);
    }

    public void render(PoseStack poseStack, T animatable, BakedGeoModel bakedModel, RenderType renderType, MultiBufferSource bufferSource, VertexConsumer buffer, float partialTick, int packedLight, int packedOverlay) {
        IStickeredModel<T> model = (IStickeredModel)this.getRenderer().getGeoModel();

        ResourceLocation texture = model.getStickerTextureResource(animatable);
    ...
    protected void renderLayer(PoseStack poseStack, T animatable, BakedGeoModel bakedModel, MultiBufferSource bufferSource,
        ...
        this.getRenderer().reRender(bakedModel, poseStack, bufferSource, animatable, renderTypeNew,
                bufferSource.getBuffer(renderTypeNew), partialTick, packedLight, OverlayTexture.NO_OVERLAY, 0XFFFFFFFF);
        poseStack.popPose();
    }
}
